...
int main(void) {
    ...
    setenv("DS4_ARGODRIVE_PRIMARY_NOCACHE","1",1);
    assert(ar_open(&r,fd[0],NULL,NULL));assert(r.count==1 && r.owns_primary);
    int owned=r.source[0].fd;assert(owned!=fd[0]);
    assert(ar_read(&r,123,1000003,dst,&actual));assert(actual==1000003);
    assert(!memcmp(dst,reference+123,1000003));
    ar_close(&r);errno=0;assert(fcntl(owned,F_GETFD)==-1 && errno==EBADF);
    assert(fcntl(fd[0],F_GETFD)>=0);
    snprintf(config,sizeof(config),"%s*1",path[1]);
    assert(ar_open(&r,fd[0],config,"2"));assert(r.count==2 && r.owns_primary);
    assert(ar_read(&r,177,1777777,dst,&actual));assert(!memcmp(dst,reference+177,1777777));
    assert(!ar_open(&r,-1,config,"2"));assert(r.invalid && !r.count);
    assert(ar_open(&r,fd[0],NULL,NULL));
    assert(!ftruncate(fd[0],200000));assert(!ar_read(&r,100000,1000000,dst,&actual));
    assert(actual==100000);ar_close(&r);assert(fcntl(fd[0],F_GETFD)>=0);
    unsetenv("DS4_ARGODRIVE_PRIMARY_NOCACHE");
    for(int i=0;i<3;i++){close(fd[i]);unlink(path[i]);}rmdir(dir);free(dst);free(reference);
    puts("PASS: 2/3-source reconstruction, exact coverage, unaligned ranges, 8 concurrent clients, genuine short reads, wrong size/duplicate/weight rejection, primary ownership, disabled state; independent primary uncached descriptor, single-source reconstruction, owned-fd closure and primary short-read rejection.");
}
